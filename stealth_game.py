...
def init_inside():
    ...
    pickups = []
    pickup_count = random.randint(8, 12)
    pickup_cells = random.sample(path_cells, min(pickup_count, len(path_cells)))
    
    for px, py in pickup_cells:
        ptype = random.choice([0, 0, 0, 1, 2])             
        pickups.append([px, py, ptype, False])
...
def reset_game():
    ...
    init_outside()
    camera_pos = [player_pos[0], player_pos[1], CAMERA_HEIGHT]
...
def update_pickups():
    global player_ammo, player_weapon_damage, player_fire_rate
    ...
    for pickup in pickups:
        ...
        if distance_2d(player_pos[0], player_pos[1], px, py) < 30:
            ...
            if ptype == 0:        
                ...
                player_fire_rate = min(2.0, player_fire_rate + 0.3)
...
def update_camera():
    ...
    target_x = player_pos[0] - math.cos(rad) * CAMERA_DISTANCE
    target_y = player_pos[1] - math.sin(rad) * CAMERA_DISTANCE
    target_z = CAMERA_HEIGHT
    
    collision_dist = CAMERA_DISTANCE
    has_collision = False
    
                        
    required_clearance = MAZE_WALL_THICKNESS / 2 + CAM_RADIUS
    
    if game_state == GAME_INSIDE:
                                         
                                                                                  
        if not camera_has_clearance(target_x, target_y, walls, required_clearance) or \
           line_intersects_wall(player_pos[0], player_pos[1], target_x, target_y):
            has_collision = True
    elif game_state == GAME_OUTSIDE:
                                              
        hx, hy, hz = HOUSE_POS
        house_clearance = WALL_THICKNESS / 2 + CAM_RADIUS
        
                                       
        steps = 20
        for i in range(steps + 1):
            t = i / steps
            check_x = player_pos[0] + (target_x - player_pos[0]) * t
            check_y = player_pos[1] + (target_y - player_pos[1]) * t
            
                                       
            if abs(check_x - hx) < HOUSE_SIZE//2 + house_clearance and \
               abs(check_y - hy) < HOUSE_SIZE//2 + house_clearance:
                                  
                door_x, door_y = hx, hy - HOUSE_SIZE//2
                if not (abs(check_x - door_x) < 65 and abs(check_y - door_y) < 70):
                    has_collision = True
                    break
    
    if has_collision:
                                    
        for d in range(CAMERA_DISTANCE, CAMERA_MIN_DISTANCE, -10):
            test_x = player_pos[0] - math.cos(rad) * d
            test_y = player_pos[1] - math.sin(rad) * d
            
            collision_free = True
            if game_state == GAME_INSIDE:
                                                                                             
                if not camera_has_clearance(test_x, test_y, walls, required_clearance) or \
                   line_intersects_wall(player_pos[0], player_pos[1], test_x, test_y):
                ...
                for i in range(steps + 1):
                    t = i / steps
                    check_x = player_pos[0] + (test_x - player_pos[0]) * t
                    check_y = player_pos[1] + (test_y - player_pos[1]) * t
                    if abs(check_x - hx) < HOUSE_SIZE//2 + house_clearance and \
                       abs(check_y - hy) < HOUSE_SIZE//2 + house_clearance:
                        door_x, door_y = hx, hy - HOUSE_SIZE//2
                        if not (abs(check_x - door_x) < 65 and abs(check_y - door_y) < 70):
        ...
        target_x = player_pos[0] - math.cos(rad) * collision_dist
        target_y = player_pos[1] - math.sin(rad) * collision_dist
    
                 
    camera_pos[0] += (target_x - camera_pos[0]) * CAMERA_SMOOTH
    camera_pos[1] += (target_y - camera_pos[1]) * CAMERA_SMOOTH
    camera_pos[2] += (target_z - camera_pos[2]) * CAMERA_SMOOTH
...
def draw_gun_firstperson():
    ...
    glPushMatrix()
    glTranslatef(0, 8, 0)
    glScalef(5 * gun_scale, 20 * gun_scale, 5 * gun_scale)
    ...
    glPushMatrix()
    glTranslatef(0, -4, 0)
    glScalef(6 * gun_scale, 12 * gun_scale, 10 * gun_scale)
    ...
    glPushMatrix()
    glTranslatef(0, 8, 3 * gun_scale)
    glScalef(4 * gun_scale, 18 * gun_scale, 4 * gun_scale)
    ...
    glPushMatrix()
    glTranslatef(0, -2, 0)
    glScalef(4 * gun_scale, 6 * gun_scale, 3 * gun_scale)
    ...
    glPushMatrix()
    glTranslatef(0, 2, -3 * gun_scale)
    glScalef(3 * gun_scale, 8 * gun_scale, 2 * gun_scale)
    glColor3f(0.2, 0.2, 0.2)
    glutSolidCube(1)
    glPopMatrix()
    ...
    glPushMatrix()
    glTranslatef(0, 18, 0)
    glScalef(4 * gun_scale, 4 * gun_scale, 4 * gun_scale)
...
def draw_enemy(enemy):
    ...
    body_color = [0.45, 0.12, 0.12] if state == 1 else [0.25, 0.25, 0.25]
    draw_roblox_character(ex, ey, angle, body_color)

                                                             
    eye_color = (1.0, 0.2, 0.2) if state == 1 else (1.0, 0.9, 0.2)
...
def render_outside():
    ...
    draw_roblox_character(player_pos[0], player_pos[1], player_angle, [0.3, 0.5, 0.9])
...
def draw_hud():
    ...
    draw_text(10, H - 30, f"HP: {player_hp}/{PLAYER_MAX_HP}")
    
          
    draw_text(10, H - 55, f"Ammo: {player_ammo}")
    
                  
    draw_text(10, H - 80, f"Damage: {player_weapon_damage}")
    draw_text(10, H - 105, f"Fire Rate: {player_fire_rate:.1f}x")
    ...
    if game_state == GAME_INSIDE:
        ...
        if exit_door_open:
            ...
            draw_text(W - 150, H - 55, "Enemies: 0/0 - EXIT OPEN")
        else:
            glColor3f(1.0, 0.8, 0.2)
            draw_text(W - 150, H - 55, f"Enemies: {len(living_enemies)} remaining")
        
                               
        meter_color = (1.0, 1.0, 0.0) if detection_meter < 70 else (1.0, 0.3, 0.0)
        glColor3f(meter_color[0], meter_color[1], meter_color[2])
        draw_text(10, H - 130, f"Detection: {int(detection_meter)}%")
        
                                                
        if detected:
            glColor3f(1, 0, 0)
            draw_text(W//2 - 50, H - 50, "DETECTED!")
    ...
    if game_state == GAME_OUTSIDE:
        ...
        glColor3f(1, 0, 0)
        draw_text(W//2 - 80, H//2, "GAME OVER")
        draw_text(W//2 - 80, H//2 - 30, "Press R to restart")
...
def keyboard_down(key, x, y):
    ...
    if key == b'r':
        ...
        if player_weapon_state == 0:
            ...
            elif player_powerups.get('gun'):
                player_weapon_state = 2
        elif player_weapon_state == 1:
            if player_powerups.get('gun'):
                player_weapon_state = 2
            else:
                player_weapon_state = 0
        else:
        ...
        if player_ammo > 0 and fire_cooldown <= 0:
            player_ammo -= 1
...
def update_enemies():
    global player_hp, game_state, last_game_state, invisibility_timer
    now = time.perf_counter()
    level_def = LEVEL_DEFS.get(current_level, LEVEL_DEFS[1])
    detection_radius = level_def.get('detection_radius', ENEMY_DETECTION_RADIUS)
    enemy_damage = level_def.get('enemy_damage', ENEMY_DAMAGE)
    for e in enemies:
        if not e.get('alive', True):
            continue
                    
        e['angle'] = (e.get('angle', 0) + e.get('rot_dir', 1) * 1.5) % 360

                                            
        if player_powerups.get('invisibility') and invisibility_timer > 0:
            player_detectable = False
        else:
            player_detectable = True

        dx = player_pos[0] - e['x']
        dy = player_pos[1] - e['y']
        dist = math.hypot(dx, dy)
        if dist <= detection_radius and player_detectable:
            ang_to_player = math.degrees(math.atan2(dy, dx))
            diff = abs(normalize_angle(ang_to_player - e['angle']))
            if diff > 180:
                diff = 360 - diff
            if diff < 30 and not line_intersects_wall(e['x'], e['y'], player_pos[0], player_pos[1]):
                e['alerted'] = True
                e['alert_timer'] = now
                                                           
                if dist <= ENEMY_ATTACK_RADIUS and now > e.get('attack_cd', 0):
        ...
        if e.get('alerted'):
            if math.hypot(player_pos[0] - e['x'], player_pos[1] - e['y']) > 25:
...
def update():
    ...
    global distraction, invisibility_timer
    if distraction:
